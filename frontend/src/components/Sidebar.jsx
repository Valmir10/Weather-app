  ...
  return (
    <div className={`sidebar-container ${isOpen ? "open" : ""}`}>
      ...
      <div className="sidebar-menu-container">
        {favorites.length === 0 ? (
          <p className="no-favorites">{String("No favorites added yet")}</p>
        ) : (
          favorites.map((fav, idx) => (
            <SidebarItem
              key={idx}
              city={fav.city?.name || String(fav.city)}
              temperature={fav.temp_c != null ? fav.temp_c : "--"}
              weather={fav.condition_text || "--"}
              time={fav.localtime || "--"}
              isActive={selectedCity === (fav.city?.name || fav.city)}
              onSelect={() => onSelect(fav.city?.name || fav.city)}
            />
          ))
        )}
      </div>
